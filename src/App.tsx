...
import DecisionForm, { getTagStyleAndIcon, STANDARD_TAGS } from "./components/DecisionForm";
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"verdict" | "proscons" | "comparison" | "swot" | "tree" | "chat">("verdict");
  const [shareSuccess, setShareSuccess] = useState(false);
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [deletedDecision, setDeletedDecision] = useState<{ decision: SavedDecision; index: number } | null>(null);
  ...
  const [isGeneratingBranch, setIsGeneratingBranch] = useState<string | null>(null);
  ...
  const handleFormSubmit = async (
    ...
    try {
      ...
      if (parentId) {
        updated = updated.map((d) => {
          if (d.id === parentId) {
            ...
            return {
              ...d,
              childIds: currentChildren.includes(newId) ? currentChildren : [...currentChildren, newId],
            };
          }
          return d;
        });
      ...
      setError(err.message || "Failed to analyze decision. Please check server logs.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleBranchOption = async (optionName: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to generate branch suggestion: status ${response.status}`);
      }
      ...
      setError(err.message || "Failed to generate branching suggestion. You can still manually state your sub-dilemma.");
  ...
  return (
    <div className="flex h-screen w-screen bg-slate-50 dark:bg-slate-950 font-sans overflow-hidden relative transition-colors duration-200" id="app-root-layout">
      {/* Mobile Backdrop */}
      {sidebarOpen && (
        <div 
      ...
      >
        {/* Brand Header */}
        <div className="p-6 flex items-center justify-between border-b border-slate-800" id="sidebar-brand-header">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-white font-bold text-base tracking-tight leading-none">The Tiebreaker</span>
              <span className="text-[10px] text-indigo-400 font-semibold tracking-wider uppercase mt-1">Analytical Consultant</span>
            </div>
          </div>
        ...
        {/* Search & Filtering */}
        <div className="px-4 pb-3 space-y-3 border-b border-slate-800/65" id="sidebar-filter-section">
          {/* Comparison Mode Toggle */}
          <div className="flex items-center justify-between bg-slate-850/60 p-2.5 rounded-xl border border-slate-800" id="comparison-toggle-container">
            <div className="flex items-center gap-2">
              <Layers className="w-4 h-4 text-indigo-400 shrink-0" />
              <div className="flex flex-col">
                <span className="text-[11px] font-bold text-slate-200 leading-tight">Comparison Mode</span>
                <span className="text-[9px] text-slate-500 font-medium">Compare up to 3 side-by-side</span>
              </div>
            </div>
        ...
        <nav className="flex-1 px-3 space-y-1.5 mt-3 overflow-y-auto" id="sidebar-nav">
          <div className="text-slate-500 text-[10px] font-bold uppercase tracking-widest px-3 mb-2 flex items-center justify-between">
            <span>{searchQuery || selectedTagFilter !== "All" ? "Filtered Results" : "Recent Analysis"}</span>
          ...
          {savedDecisions.length === 0 ? (
            <div className="text-center py-8 px-4" id="no-saved-decisions-prompt">
              <p className="text-xs text-slate-500 leading-relaxed italic">
                Your analyzed decisions will appear here for side-by-side comparison.
              </p>
            </div>
          ) : filteredDecisions.length === 0 ? (
            <div className="text-center py-8 px-4" id="no-filtered-results-prompt">
              <p className="text-xs text-slate-500 leading-relaxed italic">
            ...
            filteredDecisions.map((decision) => {
              ...
              const isCompared = isComparisonMode && selectedComparisonIds.includes(decision.id);
                ...
                >
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    ...
                    <div className="flex flex-col min-w-0 pr-2 flex-1">
                      <span className={`truncate font-semibold text-xs leading-normal ${isCompared ? "text-indigo-200 dark:text-indigo-300 font-bold" : ""}`}>
                        {decision.topic}
                      </span>
                      <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                        ...
                        {decision.parentId && (
                          <span className="text-[8px] bg-indigo-950/40 text-indigo-300 px-1 rounded border border-indigo-900/50 font-bold tracking-wide uppercase flex items-center gap-0.5" title={`Branched off: ${decision.branchedFromOption}`}>
        ...
        <div className="p-4 border-t border-slate-800 flex flex-col gap-3" id="sidebar-footer">
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">Theme</span>
            <button
      ...
      {/* Main Content Area */}
      <main className="flex-1 flex flex-col overflow-hidden" id="main-content-wrapper">
        {/* Header Bar */}
        <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between px-4 md:px-6 shrink-0 transition-colors duration-200" id="main-header">
          <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
            ...
            <h1 className="text-sm md:text-base lg:text-lg font-bold text-slate-800 dark:text-slate-100 truncate" id="active-decision-title">
              {isComparisonMode 
            ...
            {isComparisonMode ? (
              <span className="shrink-0 px-2 py-0.5 bg-indigo-50 dark:bg-indigo-950/30 text-indigo-700 dark:text-indigo-400 text-[9px] md:text-[10px] font-bold rounded-full border border-indigo-100 dark:border-indigo-900/50 flex items-center gap-1">
                <Layers className="w-3 h-3" />
                {selectedComparisonIds.length} / 3 SELECTED
              </span>
            ) : activeDecision && (
              <span className="shrink-0 px-2 py-0.5 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 text-[9px] md:text-[10px] font-bold rounded-full border border-emerald-100 dark:border-emerald-900/50 flex items-center gap-1">
          ...
          <div className="flex gap-2 shrink-0 ml-4" id="header-actions">
            {isComparisonMode ? (
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8" id="dashboard-content-panel">
          {error && (
            <div className="bg-rose-50 border border-rose-200/60 rounded-2xl p-5 flex items-start gap-3 text-rose-900" id="error-banner">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center space-y-6" id="dashboard-loading-state">
              ...
              <div className="space-y-2">
                <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Calculating Multi-Criteria Matrix</h3>
                <p className="text-xs text-slate-400 dark:text-slate-500 max-w-sm mx-auto leading-relaxed">
                  Synthesizing pros & cons, charting comparison scores, mapping strengths/weaknesses/opportunities, and evaluating confidence levels...
                </p>
              </div>
            </div>
          ) : isComparisonMode ? (
            <ComparativeDashboard
              selectedDecisions={savedDecisions.filter(d => selectedComparisonIds.includes(d.id))}
              onRemove={(id) => setSelectedComparisonIds(prev => prev.filter(item => item !== id))}
              onViewDetails={(id) => {
            ...
            <div className="space-y-8" id="active-decision-report">
              ...
              {(activeDecision.parentId || (activeDecision.childIds && activeDecision.childIds.length > 0) || savedDecisions.some(d => d.parentId === activeDecision.id)) && (
                <div className="flex flex-wrap items-center gap-2 px-4 py-2.5 bg-slate-100/60 dark:bg-slate-900/40 border border-slate-200/50 dark:border-slate-800/60 rounded-xl text-xs text-slate-500 dark:text-slate-400 font-semibold shadow-3xs" id="decision-ancestry-breadcrumb">
                  <GitFork className="w-3.5 h-3.5 rotate-180 text-indigo-500 shrink-0" />
                  
                  {/* Parent lineage link */}
                  {(() => {
                    const parent = activeDecision.parentId ? savedDecisions.find(d => d.id === activeDecision.parentId) : null;
                    if (parent) {
                      return (
                        <>
                          <button 
                  ...
                  <span className="text-slate-800 dark:text-slate-100 font-extrabold truncate max-w-[200px]" title={activeDecision.topic}>
                    {activeDecision.topic}
                  </span>

                  {/* Child branches count */}
                  {(() => {
                    const children = savedDecisions.filter(d => d.parentId === activeDecision.id);
              ...
              {/* Summary KPIs */}
              <section className="grid grid-cols-1 md:grid-cols-3 gap-6" id="summary-cards">
                <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-start gap-4 transition-colors duration-200">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Recommended Choice</div>
                    <div className="text-lg font-extrabold text-indigo-600 dark:text-indigo-400 mt-1 break-words" title={activeDecision.report.verdict.recommendedOption}>
                      {activeDecision.report.verdict.recommendedOption}
                    </div>
                    <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Highest strategic fit score</div>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-start gap-4 transition-colors duration-200">
                  ...
                  <div className="flex-1">
                    <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Confidence Level</div>
                    ...
                    <div className="w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full mt-2">
                      <div 
                        ...
                        style={{ width: `${activeDecision.report.verdict.confidence}%` }}
                      ></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-start gap-4 transition-colors duration-200">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Critical Challenge</div>
                    <div className="text-sm font-bold text-amber-800 dark:text-amber-300 mt-1 break-words" title={activeDecision.report.verdict.keyTradeoff}>
                      {activeDecision.report.verdict.keyTradeoff}
                    </div>
                    <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Primary calculated trade-off</div>
                  </div>
                </div>
              </section>

              {/* View/Format Toggle Tab bar */}
              <div className="border-b border-slate-200 dark:border-slate-800 flex overflow-x-auto whitespace-nowrap flex-nowrap gap-1 scrollbar-none" id="analysis-format-selector">
                <button
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-4 sm:p-6 md:p-8 shadow-sm transition-colors duration-200" id="tab-content-container">
                {activeTab === "verdict" && (
                  <VerdictView 
                    ...
                    childDecisions={savedDecisions.filter(d => d.parentId === activeDecision.id)}
                    onSelectDecision={(id) => {
                      setActiveId(id);
                      setActiveTab("verdict");
                    }}
                ...
                {activeTab === "swot" && (
                  <SwotView swotAnalysis={activeDecision.report.swotAnalysis} />
                )}
                {activeTab === "tree" && (
                  <DecisionTreeDiagram
                ...
                {activeTab === "chat" && (
                  <DecisionChat 
                    ...
                    onUpdateChatHistory={(newHistory) => handleUpdateChatHistory(activeDecision.id, newHistory)} 
                  />
                )}
              </div>

              {/* Unified Full Print Report - Hidden on screen, visible in print */}
              <div className="hidden print:block space-y-10 text-slate-900 bg-white p-6" id="print-only-full-report">
                {/* Header/Title Page info */}
                <div className="border-b-4 border-indigo-600 pb-4 mb-8 flex items-center justify-between">
                  <div>
                    <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">The Tiebreaker</h1>
                    <p className="text-xs text-indigo-600 font-bold uppercase tracking-wider mt-1">Analytical Decision Report</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-slate-500">Date Generated</p>
                    <p className="text-sm font-bold text-slate-800">{activeDecision.date}</p>
                  </div>
                </div>

                {/* Dilemma Header */}
                <div className="space-y-3 bg-slate-50 p-6 rounded-2xl border border-slate-200">
                  <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest">Dilemma Topic</span>
                  <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight leading-snug">&ldquo;{activeDecision.topic}&rdquo;</h2>
                  {activeDecision.preferences && (
                    <div className="pt-2 border-t border-slate-200/80 mt-2">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Stated Preferences & Context</span>
                      <p className="text-xs text-slate-700 leading-relaxed">{activeDecision.preferences}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-6">
                  <h3 className="text-lg font-bold text-slate-900 border-b pb-2 flex items-center gap-2">
                    <span className="bg-indigo-600 text-white rounded-md w-6 h-6 flex items-center justify-center text-xs font-bold">1</span>
                  ...
                  <div className="bg-indigo-50/50 p-6 rounded-2xl border border-indigo-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest">Recommended Choice</span>
                      <h4 className="text-2xl font-extrabold text-indigo-950 mt-1">{activeDecision.report.verdict.recommendedOption}</h4>
                    </div>
                    <div className="bg-white px-4 py-2 rounded-xl border border-indigo-100 text-center shrink-0">
                      ...
                      <p className="text-2xl font-black text-indigo-600">{activeDecision.report.verdict.confidence}%</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="md:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 space-y-3">
                      <h5 className="text-xs font-bold text-slate-400 uppercase tracking-wider">The Winning Case</h5>
                      <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-line">{activeDecision.report.verdict.reasoning}</p>
                    </div>
                    <div className="space-y-4">
                      <div className="bg-amber-50 p-6 rounded-2xl border border-amber-200 space-y-2">
                        <h5 className="text-xs font-bold text-amber-800 uppercase tracking-wider">Critical Challenge</h5>
                        <p className="text-xs text-amber-900 leading-relaxed">{activeDecision.report.verdict.keyTradeoff}</p>
                      </div>
                      <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 space-y-3">
                        <h5 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Suggested Actions</h5>
                        <ul className="space-y-2">
                          {activeDecision.report.verdict.nextSteps.map((step, idx) => (
                ...
                <div className="space-y-6">
                  <h3 className="text-lg font-bold text-slate-900 border-b pb-2 flex items-center gap-2">
                    <span className="bg-indigo-600 text-white rounded-md w-6 h-6 flex items-center justify-center text-xs font-bold">2</span>
                    Pros & Cons Comparison
                  </h3>
                  <div className="space-y-8">
                    {activeDecision.report.prosCons.map((optGroup, optionIdx) => (
                      <div key={optionIdx} className="space-y-3 page-break-avoid">
                        <h4 className="text-base font-bold text-slate-800 bg-slate-100 px-4 py-2 rounded-lg border">{optGroup.option}</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {/* Pros */}
                          <div className="bg-emerald-50/20 p-5 rounded-2xl border border-emerald-100 space-y-3">
                            <h5 className="font-bold text-xs text-emerald-800 uppercase tracking-wider border-b border-emerald-100 pb-1">Pros</h5>
                            <ul className="space-y-3">
                              {optGroup.pros.map((pro, idx) => (
                                <li key={idx} className="text-xs text-slate-700 leading-relaxed">
                                  <div className="flex justify-between items-center font-bold">
                                    <span className="text-slate-800">{pro.point}</span>
                                    <span className="text-[9px] bg-emerald-100 text-emerald-800 px-1.5 py-0.5 rounded uppercase tracking-wider shrink-0">{pro.impact} impact</span>
                                  </div>
                                  <p className="text-slate-500 mt-1">{pro.description}</p>
                                </li>
                              ))}
                            </ul>
                          </div>
                          ...
                          <div className="bg-rose-50/20 p-5 rounded-2xl border border-rose-100 space-y-3">
                            <h5 className="font-bold text-xs text-rose-800 uppercase tracking-wider border-b border-rose-100 pb-1">Cons</h5>
                            <ul className="space-y-3">
                              {optGroup.cons.map((con, idx) => (
                                <li key={idx} className="text-xs text-slate-700 leading-relaxed">
                                  <div className="flex justify-between items-center font-bold">
                                    <span className="text-slate-800">{con.point}</span>
                                    <span className="text-[9px] bg-rose-100 text-rose-800 px-1.5 py-0.5 rounded uppercase tracking-wider shrink-0">{con.impact} impact</span>
                                  </div>
                                  <p className="text-slate-500 mt-1">{con.description}</p>
                                </li>
                              ))}
                            </ul>
                          </div>
                ...
                <div className="space-y-6">
                  <h3 className="text-lg font-bold text-slate-900 border-b pb-2 flex items-center gap-2">
                    <span className="bg-indigo-600 text-white rounded-md w-6 h-6 flex items-center justify-center text-xs font-bold">3</span>
                ...
                <div className="space-y-6">
                  <h3 className="text-lg font-bold text-slate-900 border-b pb-2 flex items-center gap-2">
                    <span className="bg-indigo-600 text-white rounded-md w-6 h-6 flex items-center justify-center text-xs font-bold">4</span>
                    Complete SWOT Quadrants
                  </h3>
                  <div className="space-y-8">
                    {activeDecision.report.swotAnalysis.map((swot, idx) => (
                      <div key={idx} className="space-y-3 page-break-avoid">
                        <h4 className="text-base font-bold text-slate-800 bg-slate-100 px-4 py-2 rounded-lg border">{swot.option} SWOT Profile</h4>
                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-emerald-50/20 p-4 rounded-xl border border-emerald-100">
                            <h5 className="font-bold text-xs text-emerald-800 uppercase tracking-wider mb-2 border-b border-emerald-100 pb-1">Strengths (S)</h5>
                            <ul className="list-disc list-inside space-y-1">
                              {swot.strengths.map((s, i) => <li key={i} className="text-xs text-slate-700">{s}</li>)}
                            </ul>
                          </div>
                          <div className="bg-amber-50/20 p-4 rounded-xl border border-amber-100">
                            <h5 className="font-bold text-xs text-amber-800 uppercase tracking-wider mb-2 border-b border-amber-100 pb-1">Weaknesses (W)</h5>
                            <ul className="list-disc list-inside space-y-1">
                              {swot.weaknesses.map((w, i) => <li key={i} className="text-xs text-slate-700">{w}</li>)}
                            </ul>
                          </div>
                          <div className="bg-indigo-50/20 p-4 rounded-xl border border-indigo-100">
                            <h5 className="font-bold text-xs text-indigo-800 uppercase tracking-wider mb-2 border-b border-indigo-100 pb-1">Opportunities (O)</h5>
                            <ul className="list-disc list-inside space-y-1">
                              {swot.opportunities.map((o, i) => <li key={i} className="text-xs text-slate-700">{o}</li>)}
                            </ul>
                          </div>
                          <div className="bg-rose-50/20 p-4 rounded-xl border border-rose-100">
                            <h5 className="font-bold text-xs text-rose-800 uppercase tracking-wider mb-2 border-b border-rose-100 pb-1">Threats (T)</h5>
                            <ul className="list-disc list-inside space-y-1">
                              {swot.threats.map((t, i) => <li key={i} className="text-xs text-slate-700">{t}</li>)}
                            </ul>
                          </div>
                        </div>
                      </div>
            ...
            <div className="max-w-3xl mx-auto space-y-8 py-4" id="welcome-form-wrapper">
              <div className="text-center space-y-3" id="welcome-hero">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 max-w-lg mx-auto leading-relaxed">
                  The Tiebreaker evaluates your alternatives across critical dimensions, charts side-by-side matrices, and maps complete SWOT quadrants to clear your path.
                </p>
              </div>
        ...
        {showUndoToast && deletedDecision && (
          ...
          >
            <div className="flex items-center gap-2.5 flex-1 min-w-0">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Decision Deleted</p>
                <p className="text-sm font-bold truncate text-slate-100 max-w-[180px] sm:max-w-[220px]" title={deletedDecision.decision.topic}>
